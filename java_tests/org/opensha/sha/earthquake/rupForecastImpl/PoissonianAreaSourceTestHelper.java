...
public class PoissonianAreaSourceTestHelper {
    ...
    private final static ParameterChangeWarningListener testParamChangeListener =
	...
	/**
	 * Returns {@link PoissonAreaSource} based on Peer Test Set 1 Case 10 
	 * area source data. [Area source with fixed depth. Frequency magnitude 
	 * distribution expressed as truncated Gutenberg Richter with 
     * Mmax = 6.5 and Mmin=5.0. Lambda(M>=Mmin) = 0.0395, b value = 0.9. 
     * Source is modeled as uniformly distributed point sources 
     * (or approximations to point source) across the area (1 km grid spacing) 
     * at a fixed depth of 5 km. More detailed description in 
	...
	public static PoissonAreaSource getPeerTestSet1Case10AreaSource(){
		...
		double duration = 1.0;
		double Mmin = 0.0;
		return new PoissonAreaSource(reg, 
				gridResolution, 
	...
	/**
	 * Returns {@link EqkRupForecastAPI} for Peer Test Set 1 Case 10 
	 * area source using a time span of 1 year. 
	 * [Area source with fixed depth. Frequency magnitude 
	 * distribution expressed as truncated Gutenberg Richter with 
     * Mmax = 6.5 and Mmin=5.0. Lambda(M>=Mmin) = 0.0395, b value = 0.9. 
     * Source is modeled as uniformly distributed point sources 
     * (or approximations to point source) across the area (1 km grid spacing) 
	...
	/**
	 * Returns ground motion prediction equation (GMPE) for Peer Test 
	 * Set1 Case 10. (Sadigh et al 1997, PGA, standard deviation = 0.)
	 */
	public static ScalarIntensityMeasureRelationshipAPI getPeerTestSet1Case10GMPE(){
		...
		gmpe.getParameter(StdDevTypeParam.NAME).setValue(StdDevTypeParam.STD_DEV_TYPE_NONE);
		return gmpe;
	}
