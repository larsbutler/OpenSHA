...
=======
	...
	public PoissonAreaSource(Region reg, double gridResolution,
		...
		this.magFreqDists = magFreqDistsForFocalMechs.getMagFreqDistList();
		this.focalMechanisms = magFreqDistsForFocalMechs
					...
					for (int w = 0; w < mfd.getNum(); w++) {
						...
						if (mag >= minMag && prob > 0) {
							...
							if (mag < aveRupTopVersusMag.getMinX())
								...
								depth = aveRupTopVersusMag.getClosestY(mag);
							Location finalLoc = new Location(loc.getLatitude(),
									loc.getLongitude(), depth);
							rup.setPointSurface(finalLoc,
									focalMechanisms[k].getDip());
	 ...
	 */
	public double getMinDistance(Site site) {
		double dist = reg.distanceToLocation(site.getLocation()) - maxLength
				/ 2.0;
		if (dist < 0)
			dist = 0;
		return dist;
	}
>>>>>>> 3891c0fc4a0d99a6372e698ada3c8734c7e35de2
