 ...
 * <LI>componentParam - Component of shaking: GMRoti50, average horizontal
 * (original model assumes only GMRoti50, average horizontal added assuming
 * equivalence to GMRoti50 according to SHARE Report D4.2: Adjustment
...
public class CY_2008_AttenRel extends AttenuationRelationship implements
	...
	private static final double[] cn = { 2.996, 3.292, 3.514, 3.563, 3.547,
			3.448, 3.312, 3.044, 2.831, 2.658, 2.505, 2.261, 2.087, 1.812,
			1.648, 1.511, 1.47, 1.456, 1.465, 1.478, 1.498, 1.502, 2.996, 1.648 };
	...
	protected void initPropagationEffectParams() {
		...
		DoubleConstraint warn2 = new DoubleConstraint(DISTANCE_MINUS_WARN_MIN,
	...
	protected void initOtherParams() {
		...
		// Geometrical Mean (COMPONENT_AVE_HORZ) = Geometrical MeanI50
		// (COMPONENT_GMRotI50)
		StringConstraint constraint = new StringConstraint();
	...
	public double getMean(int iper, double vs30, double f_rv, double f_nm,
		...
		if (Double.isNaN(depthTo1pt0kmPerSec))
			basinDepth = Math.exp(28.5 - 3.82 * Math.log(Math.pow(vs30, 8)
					+ Math.pow(378.7, 8)) / 8);
		else
	...
	private void compute_lnYref(int iper, double f_rv, double f_nm,
		...
		double altDist = Math.sqrt(distanceJB * distanceJB + depthTop
				* depthTop);
		...
		lnYref = c1[iper]
				...
				* Math.log(rRup + c5[iper]
				...
				(cg1[iper] + cg2[iper] / Math.cosh(Math.max(mag - cg3, 0.0)))
				* rRup +
	...
	public double getStdDev(int iper, double vs30, double f_rv, double f_nm,
		...
		double sigma = sig1[iper] + 0.5 * (sig2[iper] - sig1[iper]) * magTest
				+ sig4[iper] * aftershock;
		sigma *= Math.sqrt((sig3[iper] * (1 - f_meas) + 0.7 * f_meas)
				+ (1 + NLo) * (1 + NLo));
