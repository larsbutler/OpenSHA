 ...
 * Note:
 * This class implements only the horizontal components
 * <p>
 * 
 ...
 * <LI>distanceEpicentralParameter - Epicentral distance
 * 
...
public class BindiEtAl_2010_AttenRel extends AttenuationRelationship implements
...
ParameterChangeListener {
	 ...
	 * lists.
	 */
	public BindiEtAl_2010_AttenRel(
	...
	/**
	 * Initialize Propagation Effect parameters (Joyner-Boore and Epicentral distance) and adds
	 * them to the propagationEffectParams list. Makes the parameters
	 * non-editable.
	 */
	protected final void initPropagationEffectParams() {

		distanceJBParam = new DistanceJBParameter(BindiEtAl2010Constants.DISTANCE_JB_WARN_MIN);
		...
		distanceEpicentralParameter = new DistanceEpicentralParameter(BindiEtAl2010Constants.DISTANCE_EPI_WARN_MIN);
		distanceEpicentralParameter.addParameterChangeWarningListener(warningListener);
		warn = new DoubleConstraint(
				BindiEtAl2010Constants.DISTANCE_EPI_WARN_MIN,
				BindiEtAl2010Constants.DISTANCE_EPI_WARN_MAX);
		warn.setNonEditable();
	...
	protected final void initIndependentParamLists() {
		...
		meanIndependentParams.addParameter(distanceEpicentralParameter);
	...
	public void resetParameterEventListeners() {
		...
		distanceEpicentralParameter.removeParameterChangeListener(this);
	...
	public double getMean() {
		...
		if (rJB > USER_MAX_DISTANCE && rEpi > USER_MAX_DISTANCE) {
			return VERY_SMALL_MEAN;
		} else {
			setPeriodIndex();
			double mean = 0.00;
			if (mag <= 5.5) {
				mean = getMeanRepi (iper, mag, rEpi, vs30);

			} else if (mag > 5.5){
				mean = getMeanRjb (iper, mag, rJB, vs30);
			}
			return mean;
		}
	}
	...
	public final double getStdDev() {
		...
		double stDev = 0.00;
		if (mag <= 5.5) {
			stDev = getStdDevRepi (iper, stdDevType);

		} else if (mag > 5.5){
			stDev = getStdDevRjb (iper, stdDevType);
		}
		return stDev;
	}
	 ...
	 * a) Epicentral Distance - getMeanEpi
	 */
	public double getMeanRepi(int iper, double mag, final double rEpi, final double vs30) {

		double logY = Double.NaN;

		double soilTerms = setSoilTermsEpi(iper, vs30);
		double mag_fact = (mag-BindiEtAl2010Constants.MAG_Ref);

		double term1 = BindiEtAl2010Constants.b1_epi[iper] * mag_fact; 
		
		double term2 = BindiEtAl2010Constants.b2_epi[iper] * Math.pow(mag_fact,2);
		
		double r = Math.sqrt(rEpi*rEpi + Math.pow(BindiEtAl2010Constants.h_epi[iper], 2));
		
		double term3 = (BindiEtAl2010Constants.c1_epi[iper] + 
		               BindiEtAl2010Constants.c2_epi[iper] * mag_fact) * Math.log10(r);

		logY = BindiEtAl2010Constants.a_epi[iper] + term1 + term2 + term3 + soilTerms;
		
		logY *= BindiEtAl2010Constants.LOG10_2_LN;
	 ...
	 */
	public double getMeanRjb(int iper, double mag, final double rJB, final double vs30) {
	...
	/**
	 * Get Standard Deviation for Epicentral Distance
	 */	
	public double getStdDevRepi(int iper, String stdDevType) {
		if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_NONE))
			return 0;
		else if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_INTER))
			return BindiEtAl2010Constants.LOG10_2_LN
			* BindiEtAl2010Constants.INTER_EVENT_STD_EPI[iper];
		else if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_INTRA))
			return BindiEtAl2010Constants.LOG10_2_LN
			* BindiEtAl2010Constants.INTRA_EVENT_STD_EPI[iper];
		else if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_TOTAL))
			return BindiEtAl2010Constants.LOG10_2_LN
			* BindiEtAl2010Constants.TOTAL_STD_EPI[iper];
		else
			return Double.NaN;
	}
	/**
	 * Get Standard Deviation for Epicentral Distance
	 */	
	public double getStdDevRjb(int iper, String stdDevType) {
		if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_NONE))
			return 0;
		else if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_INTER))
			return BindiEtAl2010Constants.LOG10_2_LN
	...
	public static void main(String[] args) {
		...
		BindiEtAl_2010_AttenRel ar = new BindiEtAl_2010_AttenRel(null);

		for (int i= 0; i < 2; i++){
			System.out.println("Rock ="  + BindiEtAl2010Constants.PERIOD[i]);
			System.out.println("meanEpi =" + Math.exp(ar.getMeanRepi(i, 5.0, 3, 800)));
			System.out.println("meanRjb =" + Math.exp(ar.getMeanRjb(i, 5.0, 3, 800)));
